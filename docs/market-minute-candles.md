...
- 실제 병목 후보는 긴 조회 구간의 JSON 직렬화와 네트워크 전송이다. JSON 압축으로 5거래일 응답을 282,858B에서 32,847B로 88.4% 줄였다.
- 로컬 단일 서버에서 100명 동시 조회 시 오류율은 0%였고, p95는 1일 조회 41.39ms, 5거래일 조회 136.59ms였다. 팀의 임시 합격선인 p95 200ms를 통과했다.
...
| 중앙 처리 시간 | 92.332ms |
| p95 처리 시간 | 93.705ms |
| 중앙 처리량 | 21,119,429체결/초 |
| 최저 처리량 | 20,809,924체결/초 |
...
| --- | ---: | ---: | ---: | ---: |
| 1일 | 390 | 56,658B | 6,618B | 88.3% |
| 5거래일 | 1,950 | 282,858B | 32,847B | 88.4% |

### 7.2 k6 결과

각 시나리오는 고정 VU로 20초 실행했다. 합격선은 p95 < 200ms, HTTP 오류율 < 1%, 응답 검증 성공률 > 99%다.

| 구간 | 압축 | 동시 사용자 | 요청/초 | p95 | p99 | 오류율 | 총 수신량 |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 5거래일 | 미적용 | 10 | 1,051.14 | 10.92ms | 13.23ms | 0% | 6.0GB |
| 5거래일 | gzip | 10 | 843.61 | 13.68ms | 16.12ms | 0% | 568MB |
| 5거래일 | gzip | 100 | 797.64 | 136.59ms | 221.82ms | 0% | 540MB |
| 1일 | gzip | 100 | 2,681.10 | 41.39ms | 71.31ms | 0% | 379MB |

압축은 로컬 CPU 사용으로 10 VU 처리량을 약 20% 낮췄지만, 전송량은 약 90% 줄였다. 실제 인프라에서는 회선 비용과 외부 네트워크 지연을 고려해 압축을 유지하는 편이 유리하다. 100 VU에서 긴 응답의 p99가 200ms를 넘었으므로 운영 SLO가 p99 200ms라면 1일 초기 조회, 캐시, 응답 pagination 또는 바이너리 전송을 후속 비교한다.

## 8. 사용자 증가 대응 순서

1. 차트 진입 시 기본 1일만 REST로 받고 사용자가 과거로 이동할 때 추가 구간을 요청한다.
