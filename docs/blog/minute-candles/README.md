...
> 측정 기준일은 2026년 8월 10일입니다. 화면과 API 검증에는 같은 값을 다시 만들 수 있는 `blog-local-seed`를 사용했습니다. 실제 증권사 시세가 아니며, 공급자 분석은 공식 문서, 성능 수치는 로컬 재실행 결과를 기준으로 합니다.
...
*그림 1. 로컬 Flutter 화면은 서버가 반환한 OHLCV 중 최신 60개를 그립니다. `로컬 시험 데이터` 표시는 실제 시세와 혼동하지 않기 위한 경계입니다.*

화면에서 검증할 대상은 1분봉 카드입니다. 상단의 76,300원은 기존 종목 상세용 로컬 값이고, 분봉 카드의 마지막 종가 76,770원은 아래에서 확인할 분봉 API 값입니다.
...
2026년 8월 10일, PostgreSQL 16.14에 5종목 × 390분 × 5거래일인 9,750행을 다시 넣어 측정했습니다.
...
단일 JVM thread에서 1,950,000개 체결을 재생해 1,950개 분봉을 만들었습니다. 중앙 처리 시간은 98.088ms, p95는 109.593ms였고 중앙 처리량은 초당 19,880,150체결이었습니다.

이 숫자는 네트워크 수신, JSON 해석, DB 저장을 뺀 메모리 집계 결과입니다. 전체 서버가 초당 약 1,988만 체결을 처리한다는 뜻이 아니라, 다섯 종목의 OHLCV 계산이 현재 우선 병목은 아니라는 근거로만 사용했습니다.
...
| --- | ---: | ---: | ---: |
| 1거래일, 390개 | 56,658B | 7,058B | 87.5% |
| 5거래일, 1,950개 | 282,858B | 33,306B | 88.2% |
...
*그림 2. 같은 로컬 장비에서 저장·집계·전송을 나누어 측정했습니다. 각 숫자가 포함하지 않는 범위도 이미지 하단에 함께 표시했습니다.*
...
클라이언트는 기본적으로 서울 시각의 가장 최근 거래일 시작부터 현재까지를 조회합니다. 주말이나 월요일 장 시작 전에는 직전 금요일로 시작 시각을 옮깁니다. 응답은 시간순으로 정렬한 뒤 최신 60개만 화면에 그립니다. 네트워크에서 받은 390개를 모두 같은 폭에 넣기보다 캔들 몸통을 읽을 수 있는 밀도를 유지하기 위한 선택입니다.

화면 캡처는 2026년 8월 3일부터 7일까지 만든 결정적 역사 시드를 사용했습니다. 캡처 시점인 8월 10일의 기본 범위에는 이 데이터가 들어오지 않기 때문에 캡처 빌드에만 `MARKET_CANDLE_LOOKBACK_DAYS=7`을 주었습니다. 일반 빌드의 기본값은 자동 거래일 계산이며, 화면에는 조회된 값 중 최신 60개만 그렸습니다.

API는 삼성전자 `005930`의 5거래일 분봉 1,950개를 HTTP 200으로 반환했습니다. 마지막 분봉은 `2026-08-07 15:29 KST`이며 값은 다음과 같습니다.

- 시가 76,730원, 고가 76,810원, 저가 76,700원, 종가 76,770원
- 거래량 4,733주, 체결 수 27건
- `final=true`, `revision=1`

![PostgreSQL 행과 JWT 분봉 REST API 응답의 개수와 마지막 값을 대조한 결과](assets/02-local-api-evidence.png)

*그림 3. DB와 API의 1,950개 행, 마지막 분봉의 8개 필드를 대조했습니다. Authorization 값은 출력하지 않았습니다.*
...
화면이 보인다는 사실과 데이터 규칙이 맞다는 사실은 다릅니다. PostgreSQL 통합 테스트를 다시 실행한 결과 24개가 모두 성공했고, 실패와 건너뜀은 각각 0개였습니다.

![Gradle PostgreSQL 통합 테스트 24개 성공 결과](assets/05-gradle-integration-tests.png)

*그림 4. `*IT` 24개가 6.353초에 실행됐습니다. 이 화면은 통합 테스트 결과이며 운영 장애가 없다는 보증은 아닙니다.*
...
| 조회 범위·부하 | p95 | 요청/초 | 오류율 | 결과 |
| --- | ---: | ---: | ---: | --- |
| 1일 · 10 VU | 4.959ms | 2,061 | 0% | PASS |
| 1일 · 100 VU | 52.524ms | 2,003 | 0% | PASS |
| 5일 · 10 VU | 20.016ms | 460 | 0% | PASS |
| 5일 · 100 VU, 3회 중앙값 | 218.975ms | 467.7 | 0% | **FAIL** |

5일·100 VU는 한 번의 흔들림이 아니었습니다. 세 번의 p95가 각각 226.610ms, 214.152ms, 218.975ms로 모두 200ms를 넘었습니다. HTTP 오류는 0%였고 응답 내용 검사도 100% 성공했지만, 우리가 정한 지연 시간 기준에는 실패했습니다.

![1일과 5거래일 분봉 REST API의 k6 실제 재실행 결과](assets/04-k6-load-test.png)

*그림 5. 5일·100 VU 결과는 세 번 모두 FAIL이었습니다. 성공한 결과만 남기지 않고 실패한 p95를 함께 기록했습니다.*

이 실패가 클라이언트 범위를 정하는 근거가 됐습니다. 처음에는 가장 최근 거래일만 요청하고, 그중 최신 60개를 그립니다. 사용자가 과거로 이동할 때 이전 구간을 추가로 읽는 방식이 5일 전체를 첫 화면에 보내는 것보다 네트워크와 화면 밀도 모두에 맞았습니다.

VU 100을 실제 사용자 100명과 같다고 볼 수는 없습니다. 실제 사용자는 화면을 보고 쉬는 시간이 있지만, 이번 시험은 대기 없이 요청을 반복했고 서버와 k6도 같은 Mac에서 실행했습니다. 운영 판단에는 분리된 부하 발생기와 더 긴 단계 시험, 장시간 안정성 시험이 필요합니다.
...
- 로컬 API의 OHLCV를 Flutter가 그대로 받아 최신 60개로 렌더링합니다.
- 분봉 한 개의 관계 크기는 평균 157.118B였습니다.
- 1일·100 VU는 p95 52.524ms로 통과했고, 5일·100 VU는 세 번 모두 p95 기준에 실패했습니다.
...
- 누락된 분을 차트의 시간 간격으로 보여주는 표시. 현재 차트는 받은 봉을 나란히 그립니다.
...
6. 분봉 한 개는 평균 157.118B였고, 5거래일 JSON은 gzip으로 282,858B에서 33,306B로 줄었습니다.
...
- 집계 중앙값은 98.088ms, p95는 109.593ms, 중앙 처리량은 초당 19,880,150체결이었습니다.
