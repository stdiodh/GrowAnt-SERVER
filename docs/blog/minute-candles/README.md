...
> 기존 POC 로컬 측정일은 2026년 8월 11일이고, B1 부하 재검증과 시세 공급자 공식 문서 재확인은 2026년 8월 14일에 진행했습니다. 화면과 API 검증에는 같은 값을 다시 만들 수 있는 `blog-local-seed`를 사용했습니다. 실제 증권사 시세가 아니며, 공급자 분석은 공식 문서, 성능 수치는 로컬 재실행 결과를 기준으로 합니다.
>
...
| KIS WebSocket + REST | 첫 실수집 구현 대상으로 채택·미구현 | 다음 기술 검증 경로 |
| 토스 REST 1분봉 | 화면 비교 대상으로 채택·미구현 | 공급자가 집계한 OHLCV를 붙이는 대안 |
| 로컬 REST B1 부하 재검증 | 4조건을 각 3회 실행 | 5일·100 VU만 p95 기준 3회 모두 실패 |
| 공개 다중 사용자 운영 공급자 | 미선정 | 시세 이용 계약이 먼저 필요함 |
...
한도는 응답 속도가 아니라 허용 요청량입니다. 아래 표에는 2026년 8월 14일에 공식 공개 문서로 다시 확인한 값만 적고, 공개 문서에 없는 보관기간이나 재배포 권리는 추정하지 않았습니다.

### 실제 1분봉 후보

| 후보 | 1분봉을 가져오는 방법 | 공개 문서에서 확인한 한도 | 공개 화면 사용 | 역할·현재 상태 |
| --- | --- | --- | --- | --- |
| 한국투자 KIS | `H0STCNT0` 체결 WebSocket을 직접 집계하고 당일·과거 분봉 REST로 복구 | 신규 실전은 첫 3일 3회/초, 이후 18회/초. 모의 1회/초, WebSocket 합산 41개 등록. 일별 분봉은 실전 전용이며 120개/호출·최대 1년 | 개인 시세의 제3자 제공 금지. 제휴와 KRX·코스콤 계약 필요 | **첫 실수집 구현 대상·미구현** |
...
| 토스증권 Open API | `GET /api/v1/candles?symbol=005930&interval=1m&count=200`으로 공급자가 집계한 OHLCV를 REST 조회 | 기본 100개·최대 200개/호출, `before`로 과거 페이지 이동. 차트 그룹은 클라이언트당 최대 20회/초 | 본인 매매 목적만 허용하며 외부 배포·상업적 이용 금지 | **집계 봉 REST 비교 대상·미구현** |
...
토스증권 랜딩 페이지에는 REST와 WebSocket을 함께 언급하는 홍보 문구가 있지만, 2026년 8월 14일 현재 공식 운영 개요는 **REST API만 제공**한다고 명시하고 공개 OpenAPI 명세에도 WebSocket endpoint가 없습니다. 따라서 현재 지원이 확인된 기능만 기준으로 REST 후보로 분류했습니다. 토스의 `/trades`도 당일 최근 체결 최대 50건을 조회하는 REST API라서, 이를 반복 호출해 모든 체결 스트림을 대신하면 거래가 많은 종목에서 누락될 수 있습니다.
...
**GrowAnt의 첫 실제 시세 어댑터 구현 대상은 KIS WebSocket + REST 조합으로 정했습니다.** 실제 속도가 가장 빠르다고 확인했기 때문은 아닙니다. 현재 공급자 교체 지점은 시세 조회와 개별 체결 구독을 묶은 `MarketDataProvider`이고, KIS는 실시간 체결과 당일·과거 분봉을 함께 시험할 공식 문서와 예제가 비교적 분명합니다. 현재 다섯 종목은 41개 등록 한도 안에 있고, 체결을 직접 집계해야 기존에 만든 중복·역순·늦은 데이터 처리도 실제 입력으로 검증할 수 있습니다.

여기서 채택은 공개 운영 공급자를 확정했다는 뜻이 아니라, **다음에 구현하고 검증할 기술 경로를 선택했다**는 뜻입니다.

토스 REST는 버리지 않습니다. 공급자가 집계한 1분 OHLCV를 가장 적은 코드로 화면에 붙여보는 기술 검증에는 더 단순합니다. 다만 공식 캔들 응답에는 `timestamp`, OHLCV와 통화만 있고 GrowAnt가 노출하는 `tradeCount`와 명시적인 `isFinal`은 없습니다. 모르는 값을 `0`이나 `true`로 꾸며 PostgreSQL 원본에 넣어서는 안 됩니다. 현재 문서화된 운영 API가 REST 전용이고 과거 보관기간도 공개되어 있지 않아, 체결 수집·재연결·누락 복구 구조의 첫 공급자로는 선택하지 않았습니다.
...
공개 다중 사용자 운영에 채택된 무료 개인 API는 아직 없습니다. KIS를 포함한 증권사 API는 내부·본인용 검증 후보이고, 공개 출시의 Gate 0은 KRX·코스콤 또는 증권사와의 시세 이용 계약입니다.
...
이 절은 **아직 구현하지 않은 다음 단계**입니다. 현재 로컬 결과와 섞어 완료된 기능처럼 해석하지 않습니다.

### KIS — 첫 실수집 구현 대상

1. **권리와 시장 의미를 고정합니다.** KRX 정규장, 국내주식 5종목, 1분봉만 허용하고 화면 표시·저장·재배포 범위를 서면으로 확인합니다.
2. **`KisMarketDataProvider`를 구현합니다.** 현재 `MarketDataProvider` 계약에 맞춰 시세 조회와 체결 구독을 연결하고 `market.provider=kis`에서만 선택합니다.
3. **체결 payload의 의미를 검증합니다.** 종목, 가격, 등락률, 체결 수량, 공급자 체결 시각과 sequence를 `Tick` 전 필드로 변환합니다. 날짜 없는 장중 시각은 거래일과 `Asia/Seoul`을 결합하는 규칙을 고정하고, sequence가 재연결 전후에도 같은 체결을 구분하는지 입증합니다. 안정적인 식별자가 없다면 서버 도착 순번을 임의로 만들지 않고 중복 모델부터 확장합니다.
4. **REST 복구 포트를 둡니다.** 공급자가 집계한 분봉을 읽는 중립 포트와 복구 coordinator를 추가하고, 시작·재연결 시 마지막 확정 봉보다 1분 앞에서 다시 조회합니다.
5. **현재 분의 gap을 남기지 않습니다.** REST 조회 중 WebSocket 체결을 버퍼링하고, 연결이 끊겼던 현재 bucket은 `dirty`로 표시합니다. 분이 끝난 뒤 해당 봉을 REST로 다시 대조하기 전에는 완전한 봉으로 취급하지 않습니다.
6. **더 높은 revision으로 보정합니다.** 이미 끝난 bucket을 REST가 반환한 공급자 봉과 WebSocket 집계 봉으로 대조하되 원시 이벤트 한 목록처럼 섞지 않고, 값이 달라진 경우에만 높은 revision으로 멱등 저장합니다.
7. **한 연결·한 writer로 시작합니다.** heartbeat, 등록 ACK, 토큰 갱신, 제한된 지수 backoff와 jitter, 429 `Retry-After`, 종목별 마지막 체결·확정 봉·gap을 기록합니다. 사용자 수가 늘어도 공급자 연결은 늘리지 않습니다.
8. **같은 조건에서 검증합니다.** payload 변환, 중복, 역순, 부분 구독 실패와 재연결을 자동 테스트하고, 실제 장에서 다섯 종목을 최소 한 거래일 수집해 공급자 분봉과 전체 OHLCV를 대조합니다.

### 토스 — 공급자 집계 봉 REST 비교 대상

1. `symbol`, `interval=1m`, `count`, `before`, `adjusted` 정책을 먼저 고정합니다. KIS 원시 체결 집계와 비교할 때는 `adjusted=false`를 사용합니다.
2. 기존 체결 포트에 가짜 체결을 만들지 않고 `HistoricalMinuteCandleSource` 같은 공급자 집계 봉 전용 경계를 둡니다.
3. OAuth client id·secret은 배포 secret으로 관리하고 서버의 고정 outbound IP를 허용 목록에 등록합니다.
4. 중앙 수집기가 다섯 종목의 최신 2개 봉을 5초 주기로 조회하되 종목을 1초 간격으로 round-robin 분산하면 평균·순간 모두 1 TPS입니다. 다섯 요청을 동시에 보내면 순간 5 TPS이므로 scheduler 정책에 이 차이를 명시합니다. 둘 다 현재 차트 그룹 20 TPS보다 낮지만 실제 한도는 응답 헤더와 429의 `Retry-After`를 따릅니다.
5. 진행 중 봉이 언제 처음 나타나고 언제 더 이상 바뀌지 않는지 측정합니다. 응답에 없는 `tradeCount`와 `isFinal`을 `0`·`true`로 꾸며 저장하지 않습니다.
6. 같은 종목·시각·`adjusted=false` 조건에서 KIS 자체 집계 봉과 비교합니다. 보관기간과 재배포 권리를 확인하기 전에는 운영 원본으로 승격하지 않습니다.
...
향후 합격 조건은 실제 장의 미복구 gap 0건, 재연결·백필 완료 시간 측정, 10분 단계 부하와 8시간 안정성 시험입니다. 이 조건들은 아래의 짧은 로컬 k6 기준선에 포함되지 않습니다.
...
1. **어디서 실시간 데이터를 얻을 것인가:** 현재 구현은 `sim`뿐입니다. 다음 실제 수집 검증은 KIS WebSocket 체결과 KIS REST 복구 조합으로 진행하고, 토스 REST 1분봉은 공급자 집계 봉의 화면 연결과 대조 경로로 분리합니다. 공개 서비스 공급자는 시세 재표시 계약 전에는 선정하지 않습니다.
...
다음 구현 순서는 `토스 REST 분봉으로 화면 연결을 짧게 검증`, `KIS WebSocket 수집 + REST 복구를 실제 어댑터로 검증`, `권리와 같은 조건의 실측 후 운영 공급자 확정`입니다. 이 가운데 GrowAnt의 첫 실제 수집 어댑터로 채택한 것은 KIS이고, 토스는 공급자가 집계한 봉을 읽는 REST 경로의 비교 실험입니다. 공개 운영 공급자는 아직 채택하지 않았습니다.

### 다른 사람에게 1분 안에 설명한다면

1. 체결은 실제 거래 한 건이고, 1분봉은 그 체결들을 OHLCV로 요약한 값입니다.
2. 화면과 API 증거 데이터는 실제 증권사 시세가 아니라 결정적 로컬 시드입니다.
3. 공급자가 집계한 봉을 빠르게 붙이는 토스 REST 실험과, 실시간 체결을 WebSocket으로 받고 놓친 구간을 REST로 복구하는 KIS 수집 실험은 목적이 다릅니다.
...
6. 토스증권, [Open API 안내와 이용 범위](https://home.tossinvest.com/ko/open-api), [현재 제공 기능·호출 한도](https://openapi.tossinvest.com/openapi-docs/overview.md), [캔들 API 명세](https://openapi.tossinvest.com/openapi-docs/latest/api-reference/Apis/MarketDataApi.md#getCandles)
