...
![여러 체결의 첫 가격과 마지막 가격, 최고와 최저, 거래량을 한 개의 1분봉으로 요약하는 손그림](assets/06-ticks-to-ohlcv.png)

*설명용 삽화 — 첫 가격과 마지막 가격은 몸통을, 최고·최저 가격은 꼬리를 만들고 체결 수량의 합은 거래량이 됩니다. 실제 시세나 측정 결과를 나타내지 않습니다.*
...
*설명용 삽화 — API 속도를 재기 전에 받은 시세를 저장하고 여러 사용자에게 보여줄 수 있는지부터 확인했습니다. 공급자의 실제 성능을 측정한 결과는 아닙니다.*
...
화면이 보인다는 사실과 데이터 규칙이 맞다는 사실은 다릅니다. 다시 실행한 결과 서버 단위 테스트 55개, PostgreSQL 통합 테스트 24개, Flutter 테스트 62개가 모두 성공했고 Flutter 정적 분석 문제도 0개였습니다.

![서버 단위·PostgreSQL 통합·Flutter 테스트와 분봉 회귀 검사 결과](assets/05-gradle-integration-tests.png)

*검증 결과 요약 — 서버 단위 테스트 55개, `*IT` 24개, Flutter 테스트 62개와 정적 분석을 함께 확인했습니다. 이 결과가 실제 공급자 장애나 운영 환경의 모든 문제를 보증하지는 않습니다.*
